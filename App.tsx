...
const App = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    'pix_avista' | 'pix_programado' | 'card_templo'
  >('pix_avista');
  ...
  const handleRegistrationSubmit = (e: React.FormEvent) => {
    ...
    const isValid = participants.every(
      (p) => p.name.trim() && p.ticketType && p.phone.trim()
    );
    if (!isValid) {
      ...
      <section ref={heroRef} className="relative bg-gradient-to-r from-blue-600 via-green-500 to-orange-500 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32 text-center">
          ...
          <p className="text-xl md:text-2xl mb-8 font-light drop-shadow-md">
            01 de Maio das 08h às 18h | Estância Felicidade - Brazlândia
          </p>
          <button
            onClick={() => scrollToSection(registrationRef, 'registration')}
            className="bg-white text-blue-600 px-8 py-4 rounded-full text-lg font-semibold hover:bg-blue-50 transition-transform transform hover:scale-105 shadow-lg"
          >
          ...
          <div className="bg-gradient-to-br from-blue-50 to-green-50 rounded-2xl p-8 shadow-lg min-h-[400px] flex items-center justify-center">
            {registrationSuccess ? (
              <div className="text-center">
                ...
                <p className="text-gray-600 mb-6 max-w-lg mx-auto">{successMessage.body}</p>
                <p className="text-sm text-gray-500 mb-8">(Esta é uma simulação. Nenhum pagamento real foi processado.)</p>
                <button
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => updateParticipant(index, 'ticketType', e.target.value)}
                          required
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        >
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-blue-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                      <input
                        ...
                        name="paymentMethod"
                        value="pix_avista"
                        checked={paymentMethod === 'pix_avista'}
                        onChange={() => setPaymentMethod('pix_avista')}
                    ...
                    <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-blue-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                      <input
                        type="radio"
                        name="paymentMethod"
                        value="pix_programado"
                        checked={paymentMethod === 'pix_programado'}
                        onChange={() => setPaymentMethod('pix_programado')}
                        className="w-5 h-5 text-blue-600 focus:ring-blue-500"
                      />
                    ...
                    <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-blue-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                      <input
                        ...
                        name="paymentMethod"
                        value="card_templo"
                        checked={paymentMethod === 'card_templo'}
                        onChange={() => setPaymentMethod('card_templo')}
                        className="w-5 h-5 text-blue-600 focus:ring-blue-500"
                      />
